...
export class YarnPackageManager implements INodePackageManager {
	...
	private getDependencyInformation(dependency: string): INpmInstallResultInfo {
		const scopeDependencyMatch = dependency.match(YarnPackageManager.SCOPED_DEPENDENCY_REGEXP);
		...
		if (scopeDependencyMatch) {
			...
			const matches = dependency.match(YarnPackageManager.DEPENDENCY_REGEXP);
			if (matches) {
				name = matches[1];
				version = matches[2];
			}
